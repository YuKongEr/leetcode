...
public class Solution {
    ...
    public static boolean isMatch1(String s, String p) {
        ...
        for (int i = 1; i < s.length() + 1; i++) {
            for (int j = 1; j < p.length() + 1; j++) {
                if (p.charAt(j - 1) != '*') {
                    match[i][j] =
                            match[i - 1][j - 1] && (s.charAt(i - 1) == p.charAt(j - 1) || p.charAt(j - 1) == '.');
    ...
    public boolean isMatch(String s, String p) {
        ...
        // 初始化 base case p.chatAt(j) == '*'  dp[0][j - 2] = dp[0][j]
        dp[0][0] = true;
        ...
        for (int i = 1; i < s.length() + 1; i++) {
            for (int j = 1; j < p.length() + 1; j++) {
                // case 1 p.charAt(j - 1) != '*'  dp[i][j] = (s.charAt(i - 1) == p.charAt(j - 1) || p.charAt(j - 1) == '.') && dp[i - 1][j - 1];
                if (p.charAt(j - 1) != '*') {
                    dp[i][j] = (s.charAt(i - 1) == p.charAt(j - 1) || p.charAt(j - 1) == '.') && dp[i - 1][j - 1];
                } else {
                    //  case 2 s.charAt(i - 1) == p.charAt(j - 2) 字符* 可以匹配 0个字符 1个字符 多个字符
                    if (s.charAt(i - 1) == p.charAt(j - 2) || p.charAt(j - 2) == '.') {
                        //         匹配0个           匹配1个             匹配多个
                        dp[i][j] = dp[i][j - 2] || dp[i - 1][j - 2] || dp[i - 1][j];
                    } else {
                        // case 3  s.charAt(i - 1) != p.charAt(j - 2)  dp[i][j] = dp[i][j - 2] 直接抹去q最后两个字符
                        dp[i][j] = dp[i][j - 2];
                    }
                }
            }
    ...
    public  boolean isMatch2(String s, String p) {
        ...
        if (p.length() == 1 ) {
            return s.length() == 1 && headIsMatch(s, p);
        }
        ...
        if (p.charAt(1) != '*') {
            if (s.length() == 0) {
                ...
                return headIsMatch(s, p) && isMatch2(s.substring(1), s.substring(1));
            }
            // 有*
        } else {
            while (s.length() > 0 && headIsMatch(s, p)) {
    ...
    private boolean headIsMatch(String s, String p) {
